...
def main():
    ...
    disc_lower = discriminator.HyperCtrolDiscriminator()

    if torch.cuda.is_available():
        model = model.cuda()
        disc = disc.cuda()
        disc_lower = disc_lower.cuda()
    
    optimizer = optim.Adam([p for p in model.parameters() if p.requires_grad],
                           lr=hparams.initial_learning_rate, betas=(0.5, 0.999))
    disc_optimizer = optim.Adam([p for p in disc.parameters() if p.requires_grad],
                           lr=hparams.disc_initial_learning_rate, betas=(0.5, 0.999))
    disc_lower_optimizer = optim.Adam([p for p in disc_lower.parameters() if p.requires_grad],
                           lr=hparams.disc_initial_learning_rate, betas=(0.5, 0.999))
        
    train(model, optimizer, disc, disc_optimizer, disc_lower, disc_lower_optimizer, train_dataloader)

def train(model, optimizer, disc, disc_optimizer, disc_lower, disc_lower_optimizer, train_dataloader):
    current_epoch = 0
    ...
    for epoch in range(current_epoch, hparams.nepochs):
        ...
        for step, (x, indiv_mels, mel, gt) in prog_bar:
            ...
            disc_lower.train()
            disc_lower_optimizer.zero_grad()
            ...
            pred_fake = disc_lower(face_sequences_g.detach())
            loss_D_fake = get_gan_loss(pred_fake, False)
            pred_real = disc_lower(face_sequences_gt.clone().detach())
            loss_D_real = get_gan_loss(pred_real, True)
            loss_D_lower = (loss_D_fake + loss_D_real).mean() * 0.5        
            loss_D_lower.backward()
            disc_lower_optimizer.step()
            ...
            gan_lower_loss = get_gan_loss(disc_lower(face_sequences_g), True).mean()          
            loss_G = 0.2 * perceptual_gen_loss + 0.2 * gan_loss + 0.4 * gan_lower_loss + (1 - 0.2 - 0.2 - 0.4)*l1_loss
            loss_G.backward()
            optimizer.step()
            ...
            running_disc_lower_loss += loss_D_lower.item()

            description = 'Epoch: {}, L1: {:.3f}, Percep: {:.3f}, Gen: {:.3f}, Disc: {:.3f}, DiscL: {:.3f}'.format(
                epoch,
                running_l1_loss / (step + 1),
                running_perceptual_loss / (step + 1),                
                running_gen_loss / (step + 1),
                running_disc_loss / (step + 1),
                running_disc_lower_loss / (step + 1))
            prog_bar.set_description(description)
        logger(description, f'{hparams.result_path}/log.txt')    
        if epoch % hparams.checkpoint_interval == 0:
            ...
            save_model(disc_lower, epoch, disc_lower_optimizer, f'{hparams.result_path}/discL_e{epoch}-{ct}.pt')
...
def logger(description, folder):
    os.makedirs(os.path.dirname(folder), exist_ok=True)
    with open(folder, 'a') as file:
        file.write(f'{description}\n')
          
#MultiscaleDiscriminator + CBAM
