...
def main():
    ...
    if torch.cuda.is_available():
        model = model.cuda()
    load_model(model, None, '/media/cxnam/cxnam_folder/HaftFusion/pretrained/HaftFusion_Result1_CBAM/gen_e195-2023-12-09 23:30:48.693524.pt')
...
def eval(model, test_dataloader):
    ...
    for step, (x, indiv_mels, mel, gt) in prog_bar:
        ...
        # prog_bar.set_description('SSIM: {:.3f}; PSNR: {:.3f}; FID: {:.3f}; LPIPS: {:.3f}; CPBD: {:.3f}; CSIM: {:.3f}'.format(
        ...
        prog_bar.set_description('LMD: {:.3f}; POSE: {:.3f}; CONTENT: {:.3f} '.format(
            running_lmd_score / (step + 1),
            running_pose_score / (step + 1),
            running_content_score / (step + 1),
            ))
        
        #SSIM: 0.922; PSNR: 30.264; FID: 30.689; LPIPS: 0.051; CPBD: 0.198; LMD: 0.767; POSE: 0.442; CONTENT: 1.188
