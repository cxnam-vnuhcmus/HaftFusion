...
def train(model, optimizer, disc, disc_optimizer, train_dataloader):
    ...
    for epoch in range(current_epoch, hparams.nepochs):
        ...
        for step, (x, indiv_mels, mel, gt) in prog_bar:
            ...
            #Discriminator
            pred_fake = disc(g.detach())
            loss_D_fake = get_gan_loss(pred_fake, False)
            pred_real = disc(gt.clone().detach())
            ...
            sync_loss = get_syncnet_loss(mel, g)
            gan_loss = get_gan_loss(disc(g), True).mean()            
            loss_G = 0.2 * perceptual_gen_loss + 0.2 * sync_loss + 0.4 * gan_loss + (1 - 0.2 - 0.2 - 0.4)*l1_loss
            loss_G.backward()
            optimizer.step()
            ...
            prog_bar.set_description('Epoch: {}, L1: {:.3f}, Percep: {:.3f}, Sync: {:.3f}, \
                                     Gan: {:.3f}, Gen: {:.3f}, Disc: {:.3f}'.format(
                epoch,
...
def eval(model, optimizer, disc, disc_optimizer, test_dataloader):
    if hparams.pretrained != '':
        load_model(model, optimizer, hparams.pretrained)
    if hparams.disc_pretrained != '':
        load_model(disc, disc_optimizer, hparams.load_model)
    ...
    for step, (x, indiv_mels, mel, gt) in prog_bar:
        ...
        prog_bar.set_description('L1: {:.3f}, Percep: {:.3f}, Gen: {:.3f}, Disc: {:.3f}'.format(
            running_l1_loss / (step + 1),
            running_perceptual_loss / (step + 1),
            running_gen_loss / (step + 1),
            running_disc_loss / (step + 1)))
...
def main():
    ...
    test_dataloader = data_loader.Create_Dataloader(hparams.test_file, batch_size=hparams.batch_size, num_workers=hparams.num_workers)
